...
VAR
    island     : ARRAY [index, index] OF contents;    {Wolf Island with border}
    foodunits  : ARRAY [1..size, 1..size] OF posint;  {wolves' food unit matrix}
...
PROCEDURE Initialize;
    ...
    BEGIN
        ...
        {Read the initial random number seed.}
        read(seed);
    END {Initialize};
...
PROCEDURE NewLocation (critter : contents;
                       oldrow, oldcol : index;
                       VAR newrow, newcol : index);

    {Find a new location for the critter currently at
